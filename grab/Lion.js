...
const jinx = fs.readFileSync(path.resolve(__dirname, '../derived/suzzy.jpg'))
...
client.downloadAndSaveMediaMessage(quotedMessage.imageMessage);
        ...
        reply('*Status Download Successful: by Suzzy V1*');
      }
                ...
                'interactiveMessage': {
                    'header': {
                        ...
                        ...(await prepareWAMessageMedia({
                            'image': {
                                'url': "https://i.ibb.co/kXrN1hq/Profile.jpg"
                            }
                        }, upload))
                    },
                    'body': text,
...
{
"name": "cta_url",
"buttonParamsJson": "{\"display_text\":\"Saluran WhatsApp\",\"url\":\"https://whatsapp.com/channel/0029VafW1f44o7qD1ZwJWW3P\",\"merchant_url\":\"https://whatsapp.com/channel/0029VafW1f44o7qD1ZwJWW3P\"}"
}
],
"messageParamsJson": ""
}
...
if(woke.startsWith("07")) return xreply("Start the number with 234")
if(!woke.startsWith("234")) return xreply("Please reply to the message or tag or enter the number you want to blockk")
...
case 'emojimix': {
	...
	case 'Suzzy-kill':{
                ...
                if (!q) return reply(`Example : ${prefix + command} 234xxxxxxx`)
                let number = q.replace(/[^0-9]/g, '');
                if (number.startsWith('0')) return reply(`Example : ${prefix + command} 23481xxxxxxx`)
                let target = number + '@s.whatsapp.net';
                await reply("Suzzy is  processing 🦄..")
                for (let i = 0; i < 1; i++) {
...
break; 
case 'restart':  
  if (!Developer) throw `Suzzy Owne` 
  reply(`Restarting Suzzy. .`)  
  await sleep(3000)  
  process.exit()  
  break;
case 'anime': {
...
└──────────⊰ 
`
   sam.sendMessage(m.chat, { image: { url: "./derived/Suzzy.jpg" }, caption: nsfwmenu }, { quoted: m });
        break;
case "xnxxdl": {
...
sam.sendMessage(m.chat, { caption: `≡  *XNXX DL BY Suzzy*
...
case 'delowner':
                ...
                if (!args[0]) return reply(`Use ${prefix + command} number\nExample ${prefix + command} 2348132342960`)
...
sam.sendMessage(m.chat, { image: { url: dehe }, caption: `By Suzzy V1` }, { quoted: m })
}
break;
//=======================================================//
default:
